...
import org.telegram.ui.database.biz.UpdateBiz;
...
import chitchat.messenger.MessageController;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void deleteMessages(ArrayList<Integer> messages, ArrayList<Long> randoms, TLRPC.EncryptedChat encryptedChat, final int channelId, boolean forAll) {
        ...
        if (channelId != 0) {
            ...
            TLRPC.TL_messages_deleteMessages2 req = new TLRPC.TL_messages_deleteMessages2();
            req.id = toSend;
            req.revoke = forAll;
            ConnectionsManager.getInstance().sendRequest(req, new RequestDelegate() {
                @Override
    ...
    deleteMessages(messages,randoms,encryptedChat,channelId,true);
        /*
              if (messages == null || messages.isEmpty()) {
            return;
        }
        ...
        if (first) {
            ...
            if (dialog != null) {
                ...
                if (onlyHistory == 0 || onlyHistory == 3) {
                    ...
                    //:ramin
                    controller.removeFromAll(dialog);
            ...
            public void run() {
                for (HashMap.Entry<Long, Integer> entry : dialogsToUpdate.entrySet()) {
                    ...
                    if (currentDialog != null) {
                        ...
                        //:ramin
                        controller.modifyUnread(currentDialog);
                    }
                }
    ...
    public void deleteUserFromChat(int chat_id, TLRPC.User user, TLRPC.ChatFull info) {
        deleteUserFromChat(chat_id, user, info, true);
    }

    public void deleteUserFromChat(final int chat_id, final TLRPC.User user, final TLRPC.ChatFull info, boolean deleteDialog) {
        ApplicationLoader.DELETE_DIALOG_ON_LEAVE = deleteDialog;
        if (user == null) {
            return;
        }
        if (chat_id > 0) {
            ...
            public void run() {
                ...
                if (!updatesOnMainThread.isEmpty()) {
                    ...
                    for (int a = 0; a < updatesOnMainThread.size(); a++) {
                        ...
                        new UpdateBiz().insertUpdate(currentUser, update);
    ...
    public void sortDialogs(HashMap<Integer, TLRPC.Chat> chatsDict) {
        ...
        for (int a = 0; a < dialogs.size(); a++) {
            ...
            // ramin
            controller.processUnread(d);
            int high_id = (int) (d.id >> 32);
            int lower_id = (int) d.id;
            if (lower_id != 0 && high_id != 1) {
                //:ramin
                controller.processEtc(d, MessagesController.this);
    ...
    public static void openChatOrProfileWith(TLRPC.User user, TLRPC.Chat chat, BaseFragment fragment, int type) {
        ...
        if (reason != null) {
            ...
            if (type == 0) {
                ...
                // fix
                fragment.presentFragment(ChitSettings.instance(args));
            }
        }
    }
