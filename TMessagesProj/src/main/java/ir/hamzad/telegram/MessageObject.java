...
public class MessageObject {
    ...
    public static TextPaint textPaintIn,
            textPaintOut;
    ...
    public MessageObject(TLRPC.Message message, AbstractMap<Integer, TLRPC.User> users, AbstractMap<Integer, TLRPC.Chat> chats, boolean generateLayout) {
        ...
        if (textPaintIn == null) {
            textPaintIn = new TextPaint(Paint.ANTI_ALIAS_FLAG);
            textPaintIn.linkColor = 0xff316f9f;
            textPaintIn.setColor(0xFFFFFFFF);

            textPaintOut = new TextPaint(Paint.ANTI_ALIAS_FLAG);
            textPaintOut.linkColor = 0xff316f9f;
            textPaintOut.setColor(0xFF000000);
        }

        textPaint=textPaintOut;
        /*if(isOutOwner())
            textPaint=textPaintOut;
        else textPaint=textPaintIn;*/


        textPaint.setTextSize(AndroidUtilities.dp(MessagesController.getInstance().fontSize));
        textPaint.setTypeface(FontManager.instance().getTypeface());
    ...
    private void measureInlineBotButtons() {
        ...
        if (botButtonPaint == null) {
            ...
            botButtonPaint.setTypeface(FontManager.instance().getTypeface());
        }
